...
const driveController = new Elysia({ prefix: "/drive" })
  .post(
    "/upload",
    async ({ body, status }) => {
      const { files } = body;
      ...
      for (let file of files) {
        ...
        uploaded.push({ original: original.data, optimized: optimized.data });
      }
          ...
          "201": {
            ...
            content: {
              "application/json": {
                example: {
                  files: [
                    { original: "image-drive-id", optimized: "image-drive-id" },
                  ],
                },
              },
            },
